...
namespace img
{
	...
	class EasyImage
	{
		public:
            ...
            void draw_zbuf_triag(const Vector3D& A, const Vector3D& B, const Vector3D& C, double d, double dx, double dy, std::vector<double>& fullAmbientRef, std::vector<double>& diffuseRef, std::vector<double>& specularRef, double& refCoeff, const std::vector<Light*>& lights, const Vector3D& eyeCamera, const Matrix &eyeTransf, bool shadowOn, const std::vector<Texture*>& textures, const std::vector<std::vector<double> >& uv, std::vector<Vector3D* > norm);
            ...
            void shadow_zbuf_triag(const Vector3D& A, const Vector3D& B, const Vector3D& C, Light* light) const;
